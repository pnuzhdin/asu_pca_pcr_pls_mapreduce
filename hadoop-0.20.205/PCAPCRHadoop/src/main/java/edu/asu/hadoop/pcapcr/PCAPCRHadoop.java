...
import edu.asu.hadoop.hyperspectral.CountSamplesMapper;
import edu.asu.hadoop.hyperspectral.CountSamplesReducer;
...
import edu.asu.hadoop.pca.transformation.DataToPCAMapper;
...
import edu.asu.hadoop.preparedata.PrepareHyperspectralDataMapper;
...
/**
 * PCA (covariance decomposition) or PCR (least squeares regression model) for Hadoop 0.20.205
...
public class PCAPCRHadoop
    ...
    public static boolean prepareData(
            ...
            Path inputDataPath,
            Path outputDataPath,
            boolean isHyperspectral) throws InterruptedException, ClassNotFoundException, IOException {
        Job job = new Job(conf, "edu.asu.hadoop.covariancedecomposition.PCAPCRHadoop: prepareData");

        job.setJarByClass(PCAPCRHadoop.class);

        if (isHyperspectral) {
            job.setMapperClass(PrepareHyperspectralDataMapper.class);
        } else {
...
//        return true;
     ...
     */
    public static long calculateSamplesCount(
            Configuration conf,
            Path inputDataPath,
        ...
        job.setMapperClass(CountSamplesMapper.class);
        job.setCombinerClass(CountSamplesReducer.class);
        job.setReducerClass(CountSamplesReducer.class);
        job.setNumReduceTasks(1);
        //job.setSpeculativeExecution(false);
        ...
        if (job.waitForCompletion(true)) {
            //FileSystem fs = FileSystem.get(outputDataPath.toUri(), conf);
...
//    }
    ...
    public static int componentsByEnergyThreshold(
            ...
            double threshold) {
        double fullEnergy = 0;
        final double magicNum = 100000;
        for (Double eigenValue : eigen.descendingKeySet()) {
            fullEnergy += eigenValue / magicNum;
        }
    ...
    public static boolean dataToPCA(
            ...
            Path inputDataPath,
            Path outputDataPath,
            boolean isHyperspectral) throws IOException, InterruptedException, ClassNotFoundException {
    ...
    public int run(String[] args) throws Exception {
        if (args.length < 13) {
            System.err.printf("Usage: program [generic options]\n"
                    ...
                    + "[Hyperspectral: is ignore outliers: y | n]\n"
                    + "[Hyperspectral: classes '1 2 3']\n");
        ...
        long trainingSetSize = Long.valueOf(args[4]);
        long testingSetSize = Long.valueOf(args[5]);
        ...
        final String tempDataPath = args[11];
        final String elapsedTimePath = args[12];
        final String hyperspectralImageFormat = (args.length >= 14 ? args[13] : null);
        final Boolean hyperspectralIsIgnoreOutliers = (args.length >= 15 ? "y".equals(args[14]) : null);
        final String hyperspectralClasses = (args.length >= 16 ? args[15] : null);
//        final String hyperspectralImageFormat = "<H";
//        final Boolean hyperspectralIsIgnoreOutliers = true;
//        final String hyperspectralClasses = "1 2 3";

        //FileOutputStream elapsedTimeStream = new FileOutputStream(elapsedTimePath, true);
        OutputStream elapsedTimeStream = System.out;
        elapsedTimeStream.write("-----------------\r\n".getBytes());

        Configuration conf = getConf();

        FileSystem fs = FileSystem.get(new URI(tempDataPath), conf);
        ...
        conf.setInt("mapreduce.input.hyperspectraldatainputformat.numbands", dataSize);
        if(hyperspectralImageFormat != null) {
            conf.set("mapreduce.input.hyperspectraldatainputformat.dataformat", hyperspectralImageFormat);
        }
        conf.setBoolean("mapreduce.input.hyperspectraldatainputformat.isrounddownward", true);
        if (hyperspectralIsIgnoreOutliers != null) {
            conf.setBoolean("hyperspectralIsIgnoreOutliers", hyperspectralIsIgnoreOutliers);
        }
        if (hyperspectralClasses != null) {
            conf.set("hyperspectralClasses",
                    String.valueOf(hyperspectralClasses.split(" ").length) + " " + hyperspectralClasses);
        }
        ...
        if (hyperspectralClasses != null) {
            curTime = System.currentTimeMillis();
            Path trainingSamplesCount = new Path(tempDataPath + "-trainingsamplescount");
            trainingSetSize = calculateSamplesCount(conf, trainingSetPreparedPath, trainingSamplesCount);
            conf.setLong("samplesCount", trainingSetSize);
            printTime(elapsedTimeStream, "calculateTrainingSamplesCount(" + trainingSetSize + ")", System.currentTimeMillis() - curTime);
        }
        ...
        if (isNeedDataCentring) {
            ...
            ConfigurationUtils.setVector(conf, "meanValues", meanVector);
            writeVector(conf, new Path(tempDataPath + "-meanvector"), meanVector);
            if (isNeedDataNormalizing) {
                ConfigurationUtils.setVector(conf, "stdevValues", stdevVector);
                writeVector(conf, new Path(tempDataPath + "-stdevvector"), stdevVector);
            }
        }
...
//        printTime(elapsedTimeStream, "trainingSetToPCA", System.currentTimeMillis() - curTime);
        ...
        prepareData(conf, testingSetPath, testingSetPreparedPath, hyperspectralImageFormat != null);
        printTime(elapsedTimeStream, "prepareTestingSet", System.currentTimeMillis() - curTime);

        if (hyperspectralClasses != null) {
            curTime = System.currentTimeMillis();
            Path testingSamplesCount = new Path(tempDataPath + "-testingsamplescount");
            testingSetSize = calculateSamplesCount(conf, testingSetPreparedPath, testingSamplesCount);
            printTime(elapsedTimeStream, "calculateTestingSamplesCount(" + testingSetSize + ")", System.currentTimeMillis() - curTime);
        }
